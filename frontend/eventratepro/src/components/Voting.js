...
function Voting() {
  ...
  useEffect(() => {
    const fetchQuestionnaireAndEvent = async () => {
      try {
        const res = await fetch(`http://localhost:5000/${questionnaireID}/${posterID}`);
  ...
  const ticketOptionsList = Object.entries(answers).map(([questionTitle, option]) => ({
    questionTitle,
    label: option.label,
  ...
  try {
    const res = await fetch('http://localhost:5000/submitVote', {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="voting-page" style={{ padding: "20px", fontFamily: "Arial" }}>
      <h1>Questionnaire</h1>
      {questionnaire.criteriaList?.map((criteria, index) => (
        <div key={index} className="criteria-section" style={{ marginBottom: "30px" }}>
          <h2>{criteria.title}</h2>
          {criteria.questionList?.map((question, qIndex) => (
            <div key={qIndex} className="question-block" style={{ marginTop: "15px" }}>
              <p><strong>Q{qIndex + 1}: {question.title}</strong></p>
                ...
                question.optionList.map((option, oIndex) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => handleOptionChange(question.title, option.label)}
                      style={{ marginRight: "10px" }}
                    />
                    <span>{option.label} ({option.points} pts)</span>
                  </div>
                ))
              ) : (
